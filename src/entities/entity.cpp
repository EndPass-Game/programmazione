...
Entity::Entity(int life, int attack) : Displayable(Position{1, 1}, 'E'),
                                       life_(life),
                                       attack_(attack),
                                       direction_(enums::Direction::NONE) {}

Entity::Entity(int life, int attack, Position current, char displayChar) : Displayable(current, displayChar),
                                                                           life_(life),
                                                                           attack_(attack),
                                                                           direction_(enums::Direction::NONE) {}
...
void Entity::move(manager::Level *levelManager)
{
    ...
    if (isMoving){
        enums::CollisionObject collision = levelManager->getCollisionObject(Position(new_x, new_y));
        if (enums::NONE == collision) {
            setPosition(Position(new_x, new_y));
        } else {
            handleCollision(levelManager, collision);
        }
    }
...
void Entity::handleCollision(manager::level *levelmanager, enums::Collisin collisiot){
    switch (collision) {
